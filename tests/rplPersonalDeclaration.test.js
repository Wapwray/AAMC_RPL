...
test("published page variants expose their current declaration runtime", () => {
  assert.match(mainPage, /welcomeVersionBadge">V2\.12</);
  assert.match(mainPage, /src="rpl-personal-declaration\.js"/);
  assert.match(q3Page, /WELCOME_VERSION = "V3\.5"/);
  assert.match(q3Page, /V2\\\.\(\?:12\|11\|10\|9\|8/);
  assert.match(autoTesterPage, /RUNTIME_VERSION = "2\.2"/);
  assert.match(autoTesterPage, /WELCOME_VERSION = "V2\.12"/);
  assert.match(autoTesterPage, /V2\\\.\(\?:12\|11\|10\|9\|8/);
});
