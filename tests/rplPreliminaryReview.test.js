...
test("renders approved report labels, escaped verbatim responses, and one row/article per question", () => {
  ...
  assert.match(html, /\.report\s*\{[^}]*max-width:\s*186mm;/);
  assert.match(html, /\.summary,\s*\.question-review-section,\s*\.limitations\s*\{[^}]*page-break-before:\s*always;[^}]*\}/);
  assert.match(html, /RPL Preliminary (Interview|Assessment) Review/i);
  ...
  assert.match(html, /<div class="response-box/);
  assert.match(html, /<div class="field-value/);
  assert.match(html, /<ul class="disclaimer-list">/);
  assert.match(html, /It does NOT constitute a competency decision\./);
  ...
  assert.doesNotMatch(html, /AI INTERVIEW RESPONSE/i);
  assert.match(html, /Assessor Evaluation - Status/);
  assert.match(html, /Assessor Comments/);
  assert.match(html, /to be completed by assessor/i);
  assert.doesNotMatch(html, /Final assessment outcome/i);
  assert.match(html, /Interview Outcome/);
  assert.match(html, /LIKELY SUFFICIENT/);
  assert.match(html, /ADDITIONAL EVIDENCE MAY BE NEEDED/);
  ...
  const conversationIndex = findHeadingIndex(/<h4>\s*Student and AI Interview conversation\s*<\/h4>/i, question24Index);
  const questionAskedIndex = findHeadingIndex(/<h4>\s*Question asked\s*<\/h4>/i, conversationIndex);
  const objectiveIndex = findHeadingIndex(/<h4>\s*Objective\s*<\/h4>/i, questionAskedIndex);
  ...
  assert.ok(conversationIndex > question24Index);
  assert.ok(questionAskedIndex > conversationIndex);
  assert.ok(objectiveIndex > questionAskedIndex);
...
test("renders assessor-mode sign-off fields and keeps assessor identity read-only", () => {
  ...
  // A4 print optimization: question review starts on a new page; trailing sections avoid forced breaks.
  assert.match(html, /\.question-review-section\s*\{[^}]*page-break-before:\s*always;[^}]*\}/);
  assert.match(html, /\.limitations\s*\{[^}]*page-break-before:\s*avoid;[^}]*\}/);
